...
// no-op on Android) Ã¢â‚¬â€ this one actually insets for the status bar/notch on
...
// Mirrors the actual gesture handling in the panResponder below exactly Ã¢â‚¬â€
...
// SystemCommand) to the matching overlay icon Ã¢â‚¬â€ see CURSOR_SHAPE_ICONS'
...
// moveCursorDotTo) Ã¢â‚¬â€ arrow is the one genuinely pointed shape (its actual
...
// Regenerated fresh each app launch Ã¢â‚¬â€ mirrors client/src/main/store.ts's
...
export default function App(): React.JSX.Element {
  // SafeAreaView only pads elements laid out in normal flow Ã¢â‚¬â€ the session
  ...
  // aangevraagd.") Ã¢â‚¬â€ distinct from Alert.alert, which stays reserved for
  ...
  // this stays set for the life of the session Ã¢â‚¬â€ needed to retry with a TOTP
  ...
  // Gates auto-reconnect to sessions that actually connected at least once Ã¢â‚¬â€
  ...
  // views other pcs. Session password (not persisted Ã¢â‚¬â€ regenerated on every
  ...
  // The hidden keyboard-bridge TextInput's own controlled value Ã¢â‚¬â€ always
  ...
  // Quick actions, Settings) Ã¢â‚¬â€ matches TeamViewer's mobile session bar.
  const [activePanel, setActivePanel] = useState<"quickActions" | "settings" | "chat" | "files" | "programs" | "aiBuddy" | "interactionHelp" | null>(null);
  // "Control a program" Ã¢â‚¬â€ pick one of the host's open windows, view/control
  ...
  // --- AI Buddy (local-only Ã¢â‚¬â€ this device's own OpenAI key, never touches
  ...
  async function takeAiBuddyScreenshot(): Promise<void> {
    ...
    if (!frame) {
      showToast("Kon geen screenshot maken Ã¢â‚¬â€ is het beeld al geladen?");
      return;
    }
  ...
  async function sendAiBuddyMessage(): Promise<void> {
    ...
    try {
      ...
      setAiBuddyLog((prev) => [
        ...prev,
        { role: "assistant", text: result.ok && result.reply ? result.reply : `Ã¢Å¡Â Ã¯Â¸Â ${result.error ?? "Onbekende fout."}`, timestamp: Date.now() },
      ]);
    } catch (err) {
      setAiBuddyLog((prev) => [...prev, { role: "assistant", text: `Ã¢Å¡Â Ã¯Â¸Â ${(err as Error).message}`, timestamp: Date.now() }]);
    } finally {
  ...
  // Icon-only toolbar buttons read as unclear/generic Ã¢â‚¬â€ pairing every icon
  ...
  function toolbarIcon(Icon: IconComponent, label?: string, active = false, danger = false): React.JSX.Element {
    // Flat dark-blue icon by default (no background pill) Ã¢â‚¬â€ white only
  ...
  // Persisted across launches (see the AsyncStorage load effect below) Ã¢â‚¬â€ the
  ...
  // "cursor-shape" SystemCommand), not a hot path Ã¢â‚¬â€ plain state/re-render
  ...
  // only ever see the "arrow" value from that first render Ã¢â‚¬â€ same reason
  ...
  // session.ts) Ã¢â‚¬â€ surfaced so a soft/blurry picture reads as "your
  ...
  // See docs/47seconds.md and MobileSession's own failsafeEnabled comment Ã¢â‚¬â€
  ...
  // to decode/render into Ã¢â‚¬â€ see getZoomTiers' own comment for the full
  ...
  // whatever this was AT THAT TIME forever Ã¢â‚¬â€ a ref (not useState) is required
  ...
  // Ã¢â‚¬â€ needed to size the video box correctly for the current orientation
  ...
  // match Ã¢â‚¬â€ otherwise every mapped point was off, which is exactly why the
  ...
  // branches below) Ã¢â‚¬â€ needed so AI Buddy's screenshot button can resolve
  ...
  // Pinch-to-zoom on the video (visual only Ã¢â‚¬â€ the touchÃ¢â€ â€™mouse mapping below
  ...
  function setZoom(next: { scale: number; panX: number; panY: number }): void {
    ...
    // fraction *and* the current zoom/pan transform Ã¢â‚¬â€ a pinch/pan changes
  ...
  // render buffer Ã¢â‚¬â€ far heavier than a transform, and firing that on every
  ...
  // reallocated the SurfaceView and was visible as a flicker each time Ã¢â‚¬â€
  ...
  // transform below) Ã¢â‚¬â€ it doesn't reveal more real detail unless the
  ...
  // highest quality tier Ã¢â‚¬â€ but only when the user's own preference is
  ...
  // connection's ceiling Ã¢â‚¬â€ the opposite of what zooming in should do.
  ...
  const pinchRef = useRef<{
    ...
    // so each move sends an incremental wheel delta Ã¢â‚¬â€ like a real trackpad
    // Ã¢â‚¬â€ instead of one big jump computed against the gesture's start point.
    lastCenterX: number;
    lastCenterY: number;
    // Undecided until either the finger spacing or the centroid has moved
    // enough to tell a pinch from a two-finger pan apart Ã¢â‚¬â€ deliberately a
  ...
  // The render tier locked in for the duration of the current live pinch Ã¢â‚¬â€
  ...
  // instead) Ã¢â‚¬â€ harmless either way, and would just make updates faster if
  ...
  // axis the *container's own orientation* pins Ã¢â‚¬â€ not, as a plain
  ...
  // container in both dimensions, same as a real letterbox Ã¢â‚¬â€ it's just
  ...
  // camera) Ã¢â‚¬â€ zooming in only magnifies already-decoded pixels (see the
  ...
  // for this session yet Ã¢â‚¬â€ see the onStats handler) or reports something
  // degenerate.
  ...
  // *layout* size is, not the source resolution Ã¢â‚¬â€ confirmed against the
  ...
  // Ã¢â‚¬â€ a "tier" Ã¢â‚¬â€ and use transform *only* to scale that down to whatever
  ...
  // real SurfaceView buffer reallocation Ã¢â‚¬â€ the same cost that caused
  // flicker when done every touch-move frame, see zoomLive's own history)
  // when crossing into a new tier, not continuously Ã¢â‚¬â€ bounding worst-case
  ...
  // always uses the single sharpest tier for the whole session Ã¢â‚¬â€ zero
  ...
  // The smallest tier that's still >= the given scale Ã¢â‚¬â€ i.e. "just enough"
  ...
  // FIRST (in the view's own local space), then scale around its center Ã¢â‚¬â€ so
  ...
  // given page-absolute screen point at a given scale Ã¢â‚¬â€ used both to zoom in
  ...
  // videoWrap) Ã¢â‚¬â€ the inverse of pageToContentPct's math Ã¢â‚¬â€ used to draw the
  ...
  // absolute tap-to-position) Ã¢â‚¬â€ matches TeamViewer's "Muis-modus". ---
  ...
  // The cursor's logical position (content fraction) Ã¢â‚¬â€ a plain ref, not
  ...
  // The dot's actual on-screen position Ã¢â‚¬â€ see virtualCursorRef above for
  ...
  // contentPctToLocal's doc comment Ã¢â‚¬â€ briefly relevant right after a
  ...
  function moveCursorDotTo(xPct: number, yPct: number): void {
    ...
    // (see CURSOR_SHAPE_HOTSPOTS) Ã¢â‚¬â€ offsetting by that instead of half the
  ...
  // held-still timer Ã¢â‚¬â€ a plain single-finger drag, however slowly it
  ...
  // hold-time decides it Ã¢â‚¬â€ moving early makes it "scrolling" (remote scroll,
  ...
  function moveVirtualCursor(dxPx: number, dyPx: number): void {
    ...
    // made fast movement tedious Ã¢â‚¬â€ this keeps slow movement exactly at 1:1
    ...
    // Deliberately not setVirtualCursor/setState here Ã¢â‚¬â€ this runs on every
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // "hello" re-registers this device's ID with the server's registry Ã¢â‚¬â€
    ...
    // not Firebase's own auto-display Ã¢â‚¬â€ replaces onPushOpenedApp/
  ...
  useEffect(() => {
    ...
    // back (Android has no event for it) Ã¢â‚¬â€ re-check whenever the app returns
  ...
  function addNotification(entry: NotifyEntry): void {
    // The same notification can arrive twice Ã¢â‚¬â€ once over the WebSocket (if
    // still connected) and once via push (server sends both, see
    // server/src/index.ts) Ã¢â‚¬â€ so de-dupe by the bridge's own notification id.
  ...
  // One-tap reconnect: the stored passwordHash is resent as-is Ã¢â‚¬â€ the wire
  ...
  function startViewerSession(peerId: string): void {
    ...
    const session = new MobileSession(DEFAULT_ICE_SERVERS, signaling, peerId, {
      ...
      onStats: (stats) => {
        ...
        // Architecture) setup Ã¢â‚¬â€ react-native-webrtc's ViewManager expects
        ...
        // (see startStatsLoop) Ã¢â‚¬â€ piggybacking on it here is a reliable
      ...
      onSystemCommand: (cmd) => {
        ...
        // what we requested) Ã¢â‚¬â€ always safe to trust directly, `ok` just
    ...
    // Apply the persisted quality preference to this session too Ã¢â‚¬â€ mirrors
  ...
  // Just creates the session and waits Ã¢â‚¬â€ the offer itself arrives moments
  ...
  // isn't grantable silently, by design Ã¢â‚¬â€ the phone's own user must confirm it
  ...
  function endSession(): void {
    ...
    if (sessionStartedAtRef.current != null) {
      ...
      showToast(`Sessie beÃ«indigd Ã¢â‚¬â€ duurde ${formatDuration(durationSec)}, ${filesText}.`);
    ...
    // regardless of what we do here Ã¢â‚¬â€ this just keeps this UI's toggle from
    ...
    PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        if (interactionModeRef.current === "mouse") {
          ...
          // Same double-tap window/proximity check touch-mode's zoom uses Ã¢â‚¬â€
        ...
        if (isDoubleTap) {
          ...
          // exactly like a plain single tap doubled up Ã¢â‚¬â€ see the "pending"
          ...
          // touches.length >= 2 branch above) Ã¢â‚¬â€ this was a redundant
        ...
        // Nothing sent yet Ã¢â‚¬â€ touch-modus's single finger is three-way
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (touches.length >= 2) {
          ...
          if (pinch.gestureType === "undecided") {
            ...
            // this waits for a clear signal instead of defaulting to "pan" Ã¢â‚¬â€
          ...
          // (drifting) centroid Ã¢â‚¬â€ real pinches rarely stay perfectly
        ...
        if (interactionModeRef.current === "mouse") {
          ...
          // down point Ã¢â‚¬â€ without this, a finger that trembles even 1-2px
          ...
          if (!gesture.buttonDown && gesture.dragSelectArmed) {
            // Dragging on the second tap of a double-tap Ã¢â‚¬â€ matches
        ...
        // Ã¢â‚¬â€ "EÃ©n vinger om te scrollen" vs "Lang drukken en slepen om te
        ...
        if (g.phase === "pending") {
          ...
          if (drift < 8) return; // still just jitter Ã¢â‚¬â€ keep waiting, don't decide yet
      ...
      onPanResponderRelease: (evt) => {
        ...
        if (g.phase === "dragSelecting") {
          ...
          // without ever dragging Ã¢â‚¬â€ "Lange tik voor rechtsklikken".
        ...
        // "scrolling" phase needs nothing on release Ã¢â‚¬â€ it never touched the cursor.
        touchGestureRef.current = { phase: "pending", downTime: 0, downX: 0, downY: 0 };
      },
    })
  ).current;
  ...
  // main key goes down), then releases in reverse order Ã¢â‚¬â€ same as a person
  ...
  // de desktop-app) Ã¢â‚¬â€ the host reports back via ctrl-alt-del-status if not.
  ...
  // Ã¢â‚¬â€ switching source (desktop Ã¢â€ â€ a specific window, or between windows)
  ...
  // that window to match the phone's current aspect ratio as it's rotated Ã¢â‚¬â€
      ...
      <View style={styles.aiBuddyCardInner}>
        ...
        {!openaiKeyConfigured ? (
          ...
          <>
            ...
            >
              {aiBuddyLog.length === 0 && (
                <Text style={styles.muted}>Maak een screenshot van het scherm op afstand en stel een vraag Ã¢â‚¬â€ AI Buddy helpt je stap voor stap.</Text>
              )}
              {aiBuddyLog.map((m, i) => (
      ...
      <SafeAreaView style={[styles.sessionRoot, { marginBottom: keyboardHeight }]} edges={["top"]}>
        ...
        <View style={{ flex: 1, flexDirection: "column" }}>
          ...
          {/* panHandlers live on this inner layer (not videoWrap itself) so
              that expandBtn below Ã¢â‚¬â€ a real sibling, not a descendant of the
              gesture-capturing view Ã¢â‚¬â€ can actually receive its own taps.
              ...
              ones landing on expandBtn, before its onPress ever fires Ã¢â‚¬â€
          ...
          <View style={styles.videoGestureLayer} {...panResponder.panHandlers}>
            {remoteStream &&
              (() => {
                ...
                // current zoom, chosen by getZoomTiers/pickTier) Ã¢â‚¬â€ high
                // enough to stay sharp Ã¢â‚¬â€ positioned so its *center* lands
                ...
                // a live pinch (liveTierRef) so the layout size Ã¢â‚¬â€ and thus
                // the expensive SurfaceView reallocation it triggers Ã¢â‚¬â€
                ...
                // phone orientation (see getContentRect) Ã¢â‚¬â€ deliberately
            ...
            {interactionMode === "mouse" &&
              ...
              (() => {
                ...
                // comment near virtualCursorRef) Ã¢â‚¬â€ moveCursorDotTo already
        ...
        {/* â”€â”€ Portrait: TOP bar (Vensters Â· AI Buddy Â· Opties Â· Stop) â”€â”€â”€â”€â”€ */}
        {!isLandscape && (
          <View style={[styles.portraitTopBar, { paddingTop: insets.top, paddingLeft: insets.left, paddingRight: insets.right }]}>
            {/* Vensters dropdown */}
            <TouchableOpacity
        ...
        {/* â”€â”€ Portrait: BOTTOM bar (Muis Â· Bestand Â· Snel Â· Bord Â· Chat) â”€â”€ */}
        {!isLandscape && (
          <View style={[styles.portraitBottomBar, { paddingBottom: insets.bottom, paddingLeft: insets.left, paddingRight: insets.right }]}>
            {/* Muis / Touch modus */}
            <TouchableOpacity
            ...
            >
              ...
              <Text style={[styles.portraitBottomBtnLabel, activePanel === "interactionHelp" && styles.portraitBottomBtnLabelActive]}>
            ...
            >
              ...
              <Text style={[styles.portraitBottomBtnLabel, activePanel === "files" && styles.portraitBottomBtnLabelActive]}>Bestand</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={[styles.portraitBottomBtnLabel, activePanel === "quickActions" && styles.portraitBottomBtnLabelActive]}>Snel</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={[styles.portraitBottomBtnLabel, keyboardVisible && styles.portraitBottomBtnLabelActive]}>Bord</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={[styles.portraitBottomBtnLabel, activePanel === "chat" && styles.portraitBottomBtnLabelActive]}>Chat</Text>
            </TouchableOpacity>
          </View>
        )}

        {/* â”€â”€ Landscape: existing collapsed/expanded toolbar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
            ...
            <View style={styles.modalCard}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <Text style={styles.muted}>
                  "Altijd scherp" houdt tijdens de hele sessie de scherpst mogelijke weergave aan, ook helemaal uitgezoomd Ã¢â‚¬â€ dat kost continu meer batterij/geheugen. "Gebalanceerd" (aanbevolen) verhoogt de scherpte alleen in stappen naarmate je verder inzoomt.
                </Text>
                <View style={styles.settingsRow}>
                  ...
                  return (
                    <View key={f.id} style={styles.fileRow}>
                      <Text style={styles.fileRowName} numberOfLines={1}>
                        {f.direction === "send" ? "Ã¢â€ â€˜ " : "Ã¢â€ â€œ "}
                        {f.name}
                      </Text>
        ...
        {/* Agent confirmations (e.g. Google Antigravity) must stay reachable
            even mid-session Ã¢â‚¬â€ you may be actively controlling a PC when an
      ...
      {activeTab === "home" && (
        <ScrollView contentContainerStyle={styles.scroll} style={styles.tabContent}>
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.mono}>{myId ? formatId(myId) : "Ã¢â‚¬â€"}</Text>
            <Text style={[styles.statusText, serverStatus === "connected" ? styles.statusOk : styles.statusBad]}>
              {serverStatus === "connected" ? "â—Â Verbonden met server" : serverStatus === "connecting" ? "Verbindenâ€¦" : "â—Â Niet verbonden"}
            </Text>
        ...
        <ScrollView contentContainerStyle={styles.scroll} style={styles.tabContent}>
          ...
          <View style={styles.card}>
            ...
            <Text style={[styles.statusText, accessibilityEnabled ? styles.statusOk : styles.statusBad]}>
              {accessibilityEnabled ? "â—Â Ingeschakeld" : "â—Â Niet ingeschakeld Ã¢â‚¬â€ schermdelen werkt wel, bediening niet"}
            </Text>
          ...
          <View style={styles.card}>
            ...
            <Text style={[styles.statusText, virtualKeyboardEnabled ? styles.statusOk : styles.statusBad]}>
              {virtualKeyboardEnabled ? "â—Â Ingeschakeld" : "â—Â Niet ingeschakeld Ã¢â‚¬â€ typen op afstand is beperkt"}
            </Text>
      ...
      {activeTab === "meer" && (
        <ScrollView contentContainerStyle={styles.scroll} style={styles.tabContent}>
              ...
              <>
                <TouchableOpacity onPress={() => setSessionHistoryExpanded((v) => !v)}>
                  <Text style={styles.label}>
                    {sessionHistory.length} sessie{sessionHistory.length === 1 ? "" : "s"} opgeslagen Ã¢â‚¬â€ laatste{" "}
                      ...
                      <View key={entry.id} style={styles.savedDeviceRow}>
                        <View style={styles.savedDeviceMain}>
                          ...
                          <Text style={styles.savedDeviceId}>
                            ...
                            {entry.filesTransferred > 0 ? ` Ã‚Â· ${entry.filesTransferred} bestand(en)` : ""}
                          </Text>
                        </View>
                      </View>
                    ))}
          ...
          <View style={styles.card}>
            <Text style={styles.cardTitle}>AI Buddy</Text>
            <Text style={styles.muted}>
              Koppel je eigen OpenAI-sleutel om tijdens een sessie hulp te vragen Ã¢â‚¬â€ maak een screenshot van het scherm op
...
function createStyles(theme: AppTheme) {
  ...
  return StyleSheet.create({
    ...
    // â”€â”€ Bottom tab bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    tabBar: {
      flexDirection: "row",
    ...
    // up its own dedicated row Ã¢â‚¬â€ matches TeamViewer's full-screen-video-with-
    ...
    // Flat by default Ã¢â‚¬â€ no background pill, just a colored icon+label Ã¢â‚¬â€ a
